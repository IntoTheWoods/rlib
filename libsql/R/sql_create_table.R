sql.create.table = function(file.name, overwrite=F) {
  ...
  if( ! RSQLite::dbExistsTable(db,table.name) | overwrite ) {
    ...
    table.dataframe = read.table(file.name,header=T, sep="\t", quote="", comment.char="", stringsAsFactors=F, check.names=F)
    colnames(table.dataframe) = r2sql(colnames(table.dataframe))
    ...
    ## BROKEN create index
    ## goal is to not use the grepl statement below and identify the minimum number of columns that give a unique index
