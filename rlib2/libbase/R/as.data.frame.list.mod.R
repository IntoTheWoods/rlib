...
#'     as.data.frame(test6)
as.data.frame.list.mod <- function(x, row.names=NULL, optional=FALSE, ...) {
...
#      }
#    }
#    df <- as.data.frame(df, row.names=1:length(x), stringsAsFactors=F)
#    names(df) <- paste('Col', 1:ncol(df), sep='')
    ...
    for(i in (1:length(x))) {
      y <- x[[i]]
      if(length(y) < ncols) {
        y <- c(y, rep(NA, ncols - length(y)))
      }
        df <- rbind(df, y)
    }
    
    df <- as.data.frame(df, row.names=0:length(x),stringsAsFactors=F)
