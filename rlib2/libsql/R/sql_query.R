sql.query = function(sql, bind.data=NULL, row.names=T, db=parent.frame()$db) {
  ...
  #if (!exists("db", envir=parent.frame(), inherits=F) || ! RSQLite::dbIsValid(db) ) {
  ...
  for (word in words) {
    if (word %in% c("join","JOIN","from","FROM") & ! substr(words[counter+1],1,1) %in% c("(") ) {
      ...
      if(! test.table %in% tables & exists(test.object) && is.data.frame(get(test.object))) {
        ...
        test.tables = c(test.tables,test.table)
